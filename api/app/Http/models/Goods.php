...
class Goods extends BaseModel
{
    ...
    public static function add_fx($data,$invite_code)
    {
        ...
        try {
            $moeny=$user['money'];
            foreach ($data as $k=>$v){
                $fx_goods=self::where('goods_id',$v)->first();
                if((float)$fx_goods['fx_money']>0){
                    $moeny+=(float)$fx_goods['fx_money'];
                }else{
                    Log::error("分销错误".$fx_goods['fx_money']);
                    log::Error($data);
                }
            }
    ...
    public static function editStock($data)
    {
        foreach ($data as $k => $v) {
            ...
            $goods = self::with('goodsSku')->where('goods_id', $v['goods_id'])->first()->toArray();

            if (isset($v['sku_id'])) {
                Log::error("goods_infor");
                Log::error($goods);
                Log::error("goods_infor");
                $list=json_decode($goods['goods_sku']['json'],JSON_UNESCAPED_UNICODE);

                Log::error("list_goods");
                Log::error($list);
                Log::error("list_goods");
                if(is_array($list))
                    foreach ($list['list'] as $key => $value) {
                        if ($v['sku_id'] == $value['id']) {
                            $list['list'][$key]['stock_num'] = $value['stock_num'] - $v['number'];
                            if ($list['list'][$key]['stock_num'] >= 0) {
                                $json=json_encode($list, JSON_UNESCAPED_UNICODE);
                                Log::error("多规格");
                                Log::error($json);
                                $res=GoodsSku::where(['goods_id'=>$v['goods_id']])->update(['json'=> $json]);
                                Log::error($res);
                                Log::error("多规格");
                ...
                if ($goods['stock'] >= 0) {
                    self::where(['goods_id'=>$v['goods_id']])->update(['stock'=>$goods['stock']]);
