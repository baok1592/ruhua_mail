...
class User extends BaseModel
{
    ...
    {
        return $this->belongsTo('App\Http\models\UserLevel', 'level_id', 'id');
    }
    ...
    public function editUser($post){
        ...
        try {
            ...
            if($user['level_id']!=$post['level_id']){
                $user->level_id=$post['level_id'];
                $data['uid']=$post['id'];
                $data['type_name']='level';
                $levelname=UserLevel::find($post['level_id']);
                $data['content']=$levelname['name_l'];
                MoneyLog::create($data);
            }
            $res = $user->save();
            Db::commit();
