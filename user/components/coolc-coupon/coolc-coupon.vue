<template>
    <view class="coupon-item">
		...
		<view class="get-btn" v-if="types && item.is_choose" :style="{color:color, borderColor:color, background:solid,'background-color':'#BEBEBE'}" @click="choose(item)">已选择</view>
		<view class="get-btn" v-if="types && !item.is_choose" :style="{color:color, borderColor:color, background:solid}" @click="choose(item)" >选择使用</view>
...
export default {
	...
	methods: {
		choose(item){
			...
			this.is_chose = item.is_choose
			this.$emit('choose_done',item)
		}
